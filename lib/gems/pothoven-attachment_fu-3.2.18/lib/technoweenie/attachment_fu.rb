      ...
      protected
        ...
        def attachment_attributes_valid?
          ...
          if Object.const_defined?(:I18n) # Rails >= 2.2
            ...
            errors.add :size, I18n.translate("errors.messages.less_than", count: attachment_options[:max_size]) if send(:size).try(:>, attachment_options[:max_size])
            errors.add attr_name, I18n.translate("errors.messages.greater_than", count: attachment_options[:min_size]) if send(:size).try(:<, attachment_options[:min_size])
          else
            ...
            errors.add :size, ActiveRecord::Errors.default_error_messages[:less_than] if send(:size).try(:>, attachment_options[:max_size])
            errors.add :size, ActiveRecord::Errors.default_error_messages[:greater_than] if send(:size).try(:<, attachment_options[:min_size])
          end
        end
